...
class SnapshotCapturer(private val connection: ServerConnection, private val bucketsPerInterval: Int = 8) {
    ...
    private fun captureInbox(item: com.jvmguard.data.user.InboxItem): InboxItemDto = InboxItemDto().apply {
        id = item.id ?: 0L
    ...
    private fun captureSnapshotFile(file: com.jvmguard.data.file.SnapshotFile): SnapshotFileDto = SnapshotFileDto().apply {
        id = file.id ?: 0L
        vmId = file.vm.id
        type = file.type.name
    ...
    private fun toTelemetryType(dto: TelemetryTypeDto): TelemetryType = TelemetryType(
        ...
        runCatching { com.jvmguard.agent.config.telemetry.TelemetryUnit.valueOf(dto.unit) }
            .getOrDefault(com.jvmguard.agent.config.telemetry.TelemetryUnit.PLAIN), dto.scale)
