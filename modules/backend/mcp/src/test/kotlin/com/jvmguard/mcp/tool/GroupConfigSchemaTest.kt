...
class GroupConfigSchemaTest {
    ...
    fun serverConfigBeansExposeFullyQualifiedClassNames() {
        ...
        assertEquals("com.jvmguard.data.config.triggers.PolicyTrigger", policySection["className"])

        val triggerClassNames = list("triggerTypes").map { it["className"] }
        assertTrue(triggerClassNames.contains("com.jvmguard.data.config.triggers.PolicyTrigger"))

        val actionClassNames = list("actionTypes").map { it["className"] }
        assertTrue(actionClassNames.contains("com.jvmguard.data.config.triggers.actions.RecordJfrAction"))
    }
