...
For each SSO provider, the environment variable name is derived from the provider's display name, converted to uppercase with non-alphanumeric characters replaced by underscores:
...
For example, a provider named "Google" uses `JVMGUARD_SSO_GOOGLE_CLIENT_ID` and `JVMGUARD_SSO_GOOGLE_CLIENT_SECRET`. A provider named "Company Okta" uses `JVMGUARD_SSO_COMPANY_OKTA_CLIENT_ID` and `JVMGUARD_SSO_COMPANY_OKTA_CLIENT_SECRET`.
