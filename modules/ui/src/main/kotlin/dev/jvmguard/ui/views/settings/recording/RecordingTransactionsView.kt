...
class RecordingTransactionsView : AbstractRecordingSettingsView() {
    ...
    override val overrideLabel: String get() = t("recording.settings.transactions.override")

    private val retransformation =
        EnumSelect(t("recording.settings.transactions.retransformation"), RetransformationType::class.java).apply {
    ...
    override fun onSelectionChanged(selection: VmIdentifier) {
        ...
        content.removeAll()
        content.add(tabSheet, Span(t("recording.settings.transactions.orderHint")).apply { addClassName("jvmguard-field-hint") }, retransformation)

        refreshToolbar = {
            val add = Button(t("recording.settings.transactions.add"), VaadinIcon.PLUS.create()) {
    ...
    private fun setSpec(
        ...
        SetSpec(
            ...
            addSubtitle = t("recording.settings.set.addSubtitle.transaction"),
            saveSubtitle = t("recording.settings.set.saveSubtitle.transaction"),
            loadSets = {
