...
import dev.jvmguard.demo.server.mbean.DemoMBean;
import dev.jvmguard.demo.server.services.*;
...
public class DemoService {
    ...
    private static void registerMBeans(TrafficProfile profile, String role) {
        ...
        try {
            server.registerMBean(profile, new ObjectName("dev.jvmguard.demo:type=TrafficProfile,name=" + ObjectName.quote(role)));
            server.registerMBean(new DemoMBean(), new ObjectName("dev.jvmguard.demo:type=DemoService,name=" + ObjectName.quote(role)));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
