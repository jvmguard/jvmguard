...
import com.jvmguard.agent.servers.common.ServerClassLoader;
...
public class ClassLoaderCallee {
    ...
    public static boolean useSystemClassLoader(String name) {
        if (name.startsWith("com.jvmguard.agent") && !name.startsWith(ServerClassLoader.SERVER_PACKAGE) && !name.startsWith("com.jvmguard.agent.bootstrap.")) {
