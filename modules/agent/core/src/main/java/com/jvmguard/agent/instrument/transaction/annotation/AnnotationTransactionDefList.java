...
import com.jvmguard.agent.callee.DevOpsHandler;
...
public class AnnotationTransactionDefList extends TransactionDefList<AnnotatedTransactionDef, AnnotationDefinition, AnnotationHandler> {
    public static AnnotationTransactionDefList create(AnnotationDefinition annotationDefinition) {
        if (annotationDefinition instanceof DevOpsAnnotationDefinition) {
            return new DevOpsTransactionDefList(annotationDefinition);
        } else {
    ...
    private static class DevOpsTransactionDefList extends AnnotationTransactionDefList {
        private DevOpsTransactionDefList(AnnotationDefinition definition) {
            super(definition);
        }

        @Override
        public AnnotationHandler getHandler(DefinitionSite definitionSite) {
            Logger.log(Subsystem.INSTRUMENTATION, 6, false, "checking devops intercepted class %s %s\n", definitionSite, transactionDefs);
            ...
            for (int i = 0; i < transactionDefs.size(); i++) {
                ...
                if (transactionDef.matches(definitionSite)) {
                    ...
                    if (transactionDef.isDiscard()) {
                        ...
                        return new DevOpsHandler(transactionDef);
                    }
                } else {
