...
import com.jvmguard.integration.tests.jvmguard.mbean.mxbeans.Complex1Sub
import com.jvmguard.integration.tests.jvmguard.mbean.standard.StandardComponent
...
class MBeanTest : JvmGuardTest() {
    ...
    override fun modifyInitialRootConfig(rootConfig: GroupConfig) {
        rootConfig.telemetrySettings.mbeanTelemetries.add(MBeanTelemetryConfig("mbean tel1", TelemetryUnit.BYTES, 0, true, false).apply {
            lines.add(MBeanLineConfig("com.jvmguard.test:type=Child", "Complex/test1", ""))
        })
    }
    ...
    private fun checkStandard(serverConnection: TestServerConnection, vm: VM) {
        ...
        if (vmConfig.isAtLeastJava(15)) {
            assertEqual(
                errorMessage,
                """Cannot invoke "com.jvmguard.integration.tests.jvmguard.mbean.standard.StandardComponent.getVal()" because "y" is null"""
            )
        } else {
