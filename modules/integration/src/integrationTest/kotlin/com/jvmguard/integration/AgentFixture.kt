...
class AgentFixture {
    ...
    private fun launchWorkload(test: JvmGuardTest, jdk: JdkUnderTest, workDir: File, vmNo: Int, runNo: Int): Process {
        ...
        val cmd = buildList {
            ...
            add("-Djvmguard.int.devOpsClass=" + ClassTransaction::class.java.name)
            add("-Djvmguard.int.devOpsMethod=" + MethodTransaction::class.java.name)
            add("-Djvmguard.int.devOpsNo=" + NoTransaction::class.java.name)
            add("-Djvmguard.int.devOpsTelemetry=" + Telemetry::class.java.name)
